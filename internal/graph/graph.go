...
func (g *DependencyGraph) AddProvider(provider *registry.Descriptor) error {
	if provider == nil {
	...
	dependencies := make([]NodeKey, 0, len(provider.Dependencies))
...
func (g *DependencyGraph) CalculateDepths() {
	...
	// Start from roots (nodes with no dependencies)
	queue := make([]*Node, 0)
	for _, node := range g.nodes {
		if node.InDegree == 0 {
			node.Depth = 0
