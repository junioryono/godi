...
// buildDependencies creates registry.Dependency objects from ParameterInfo.
func (a *Analyzer) buildDependencies(info *ConstructorInfo) []*registry.Dependency {
	deps := make([]*registry.Dependency, 0, len(info.Parameters))
...
func (a *Analyzer) GetDependencies(constructor any) ([]*registry.Dependency, error) {
...
func (a *Analyzer) isInOutType(t, target reflect.Type) bool {
	...
	// For our purposes, check by name as a fallback
	// This handles cases where the types are from different packages
	return strings.HasSuffix(t.String(), ".In") || strings.HasSuffix(t.String(), ".Out")
}
